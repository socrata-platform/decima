...
import com.socrata.decima.config.DecimaLibConfig
...
object MigrateSchema extends App with Logging {
  ...
  val helpMessage =
    """MigrateSchema expects 1 or 2 parameters in the form:
      |   java -cp path/to/decima-assembly.jar com.socrata.decima.util.MigrateSchema command [number]
      |- commands: migrate, undo, redo
      |- number: number of migrations to undo or redo (ignored for migrate command)
    """.stripMargin

  val config = new DecimaLibConfig(ConfigFactory.load)
